  ...
  function handleDaftar() {
    ...
    daftar.mutate(selectedNomor, {
      onSuccess: (data) => {
        ...
        setLogs((prev) =>
          [{ time, taskId: "farmasi" as const, kodeBooking: detail?.kodeBooking ?? "—", ok: data.ok, latencyMs: data.latencyMs }, ...prev].slice(0, 50),
        );
      },
    });
  }
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-14 gap-3">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-xl flex items-center justify-center bg-[#00875A]">
                <svg
      ...
      >
        ...
        >
          ...
          <div
            ...
            style={{ background: pendingTaskId === 99 ? "#FEE2E2" : "#E3F5EF" }}
          >
            {pendingTaskId === 99 ? (
              ...
              <svg className="w-6 h-6 text-[#00875A]" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
          ...
          >
            {pendingTaskId === 99 ? "Konfirmasi Tidak Hadir / Batal" : `Konfirmasi Kirim Task ${pendingTaskId}`}
          </h3>

          {/* Deskripsi */}
          <p className="text-sm text-slate-500 text-center mb-4 leading-relaxed">
            {pendingTaskId === 99
              ? <>Pasien <strong className="text-slate-700">{detail.namaPasien ?? detail.nomorAntrian}</strong> akan ditandai <strong className="text-red-600">Tidak Hadir / Batal</strong>. Tindakan ini akan dikirim ke BPJS dan tidak dapat dibatalkan.</>
              : <>Kirim <strong className="text-slate-700">Task {pendingTaskId}</strong> untuk pasien <strong className="text-slate-700">{detail.namaPasien ?? detail.nomorAntrian}</strong> ke server BPJS Kesehatan?</>
            }
          </p>
          ...
          >
            Kode Booking: <span className="font-bold text-slate-700">{detail.kodeBooking}</span>
          </div>
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              style={{ background: pendingTaskId === 99 ? "#DC2626" : "#00875A" }}
            >
              Ya, Kirim
            </button>
          </div>
